...
if (k / 3 >= pgm_read_word_near(angreducido2 + 0)) {
angulo += 255;
};
contaang=0;
...
numled = pgm_read_byte(PolarRedu2 + k+1) - 1 + pgm_read_byte(offset1 + 0);
} else {
//inverso es cero
numled = pgm_read_byte(num_leds + 0) - pgm_read_byte(PolarRedu2 + k+1) + pgm_read_byte(offset1 + 0);
}
LedColour = pgm_read_byte(PolarRedu2 + k+2);
//color option 0
vred = 0;
vgreen = 0;
vblue = 0;
...
leds[numled].r = vblue;
k += 3;
if (k >= pgm_read_word_near(sizePolarRedu2 + 0)) {
angulo = 999;
}else{
angulo = pgm_read_byte(PolarRedu2 + k);
if (k / 3 >= pgm_read_word_near(angreducido2 + 0)) {
...
if (tiempoDibujo > tiempoescritura) {
delayMicroseconds(174 + tiempoDibujo + tiempoescritura * pgm_read_word_near(numpasos2 + 0) / (360 - pgm_read_word_near(numpasos2 + 0)));
} else {
delayMicroseconds(174 + tiempoDibujo * 360 / (360 - pgm_read_word_near(numpasos2 + 0)));
}
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
